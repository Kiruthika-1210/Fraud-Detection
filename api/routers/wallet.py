...
@router.post("/create")
def create_wallet(req: CreateWalletRequest, current_user=Depends(get_current_user)):
    wallet = db_service.create_wallet(current_user["id"], req.balance)
    if not wallet:
...
def transfer_funds(req: TransferRequest, current_user=Depends(get_current_user)):
    ...
    last_10_min_txns = db_service.get_last_10_min_txns(user_id)
    user_avg, user_std = db_service.get_user_txn_stats(user_id)
